...
import {
    ...
    STUDENT_TEMPLATE_HEADERS, STUDENT_TEMPLATE_SAMPLE,
} from '@/lib/csvParser';
...
export default function StudentImportPage({ params }: { params: Promise<{ school_id: string }> }) {
    ...
    const downloadTemplate = () => {
        const csv = generateCSV(STUDENT_TEMPLATE_HEADERS, STUDENT_TEMPLATE_SAMPLE.map(r =>
            Object.fromEntries(STUDENT_TEMPLATE_HEADERS.map(h => [h, r[h as keyof typeof r] as string ?? '']))
        ));
    ...
    const handleFile = useCallback((f: File) => {
        ...
        reader.onload = (e) => {
            ...
            setPreview({ headers, rows: rows.slice(0, 5) }); // preview first 5 rows
        };
        reader.readAsText(f);
    }, []);
        ...
        <div className="animate-fade-in">
            <div className="page-header">
                <div>
                    ...
                    <p className="page-subtitle">Upload a CSV file to admit multiple students at once</p>
                </div>
                <div style={{ display: 'flex', gap: '0.75rem' }}>
                    <button onClick={downloadTemplate} className="btn btn-ghost">⬇️ Download Template</button>
                    {schoolId && <Link href={`/dashboard/${schoolId}/students/add`} className="btn btn-ghost">➕ Add Single Student</Link>}
                </div>
            </div>

            {/* Instructions */}
            <div className="card" style={{ marginBottom: '1.5rem', background: 'var(--primary-light)', border: '1px solid rgba(59,130,246,0.2)' }}>
                <div style={{ fontWeight: 700, marginBottom: '0.75rem', color: 'var(--primary)' }}>📋 How to use Bulk Import</div>
                <ol style={{ paddingLeft: '1.25rem', fontSize: '0.875rem', color: 'var(--text-secondary)', display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                    <li>Click <strong>Download Template</strong> to get the CSV file with correct columns.</li>
                    <li>Fill in student data — one student per row. Class name must match exactly (e.g. "10").</li>
                    <li>Save as <code>.csv</code> and upload below.</li>
                    <li>Preview your data, then click <strong>Import Students</strong>.</li>
                    <li>A row-by-row import report shows successes and any errors.</li>
                </ol>
            </div>
            ...
            {!result && (
                <div
                    ...
                    {file ? (
                        <>
                            <div style={{ fontWeight: 700, color: 'var(--primary)', marginBottom: '0.25rem' }}>{file.name}</div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                        <div>
                            ...
                            <div style={{ fontSize: '0.8125rem', color: 'var(--text-muted)' }}>
                                Showing {preview.rows.length} of {file ? 'all' : '?'} rows
                            </div>
                        </div>
                    ...
                    <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                        <div style={{ padding: '1rem 1.5rem', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between' }}>
                            ...
                            <div style={{ display: 'flex', gap: '0.75rem' }}>
                                ...
                                {schoolId && <Link href={`/dashboard/${schoolId}/students`} className="btn btn-primary btn-sm">View Students</Link>}
                            </div>
                        </div>
                        <table className="data-table">
