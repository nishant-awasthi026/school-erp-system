            ...
            <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                <div className="table-wrapper">
                    <table className="data-table">
                        ...
                        <tbody>
                            {employees.length === 0 ? (
                                <tr><td colSpan={9} style={{ textAlign: 'center', padding: '3rem', color: 'var(--text-muted)' }}>No teachers yet</td></tr>
                            ) : employees.map((e: any) => (
                                <tr key={e.id}>
                                    <td>
                                    ...
                                    <td>
                                        <span className={`badge ${e.isActive ? 'badge-green' : 'badge-red'}`}>
                                            {e.teacherProfile?.status || (e.isActive ? 'ACTIVE' : 'INACTIVE')}
                                        </span>
                                    </td>
                                    <td>
                                        <Link href={`/dashboard/${school_id}/employees/${e.id}`} className="btn btn-ghost btn-sm">View</Link>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
